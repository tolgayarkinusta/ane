...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        # --- YENİ: Jitter Filtresi (EMA) ve Sıçrama Kontrolü Değişkenleri ---
        ...
        # EMA ve cooldown durumlarını saklamak için dahili değişkenler
        self.ema_cx, self.ema_cy = None, None
        self.ema_w, self.ema_h = None, None
    ...
    def reset_pid_state(self):
        ...
        # Jitter filtresi tamponlarını sıfırla
    ...
    def update_frame(self):
        ...
        try:
            ...
            # --- TEMEL ARAYÜZ ÇİZİMLERİ (DEĞİŞTİRİLMEDİ) ---
            ...
            prioritized_target_detection = None

            # --- MODEL SEÇİMİ VE YOLO TESPİTİ (DEĞİŞTİRİLMEDİ) ---
            ...
            if self.is_target_active and current_yolo_model is not None:
                ...
                if self.current_tracked_target_class is not None and not self.target_destroyed:
                    min_reacquisition_dist = float('inf')
                    last_known_center_x = self.current_tracked_target_bbox[0] + self.current_tracked_target_bbox[2] / 2
                    last_known_center_y = self.current_tracked_target_bbox[1] + self.current_tracked_target_bbox[3] / 2
                    ...
                    for det in detections:
                        if det['class_name'] == self.current_tracked_target_class:
                            ...
                            distance = np.sqrt(
                                (det_center_x - last_known_center_x) ** 2 + (det_center_y - last_known_center_y) ** 2)

                            if distance < min_reacquisition_dist and distance < self.MAX_REACQUISITION_DISTANCE_PIXELS:
                                min_reacquisition_dist = distance
                                prioritized_target_detection = det  # Öncelikli hedefi bulduk ve hafızaya aldık.

                # --- AŞAMA 3 QR KODU MANTIĞI (SİZİN KODUNUZ, DEĞİŞTİRİLMEDİ) ---
                if self.active_task == 'task3' and self.waiting_for_new_engagement_command and not self.is_ready_to_engage_from_qr:
                    ...
                    if data and data in self.qr_degrees:
                        ...
                        if closest_target_to_qr:
                            ...
                            self.waiting_for_new_engagement_command = False
                            self._update_status_label(
                                f"Durum: QR '{data}' okundu. Hedef '{self.current_tracked_target_class}' kilitlendi.")
                            self.target_info_label.setText(
                                f"Hedef: {self.current_tracked_target_class}. Açıya dönülüyor.")

                # ##################################################################
                # ############# YENİ "HEDEF SADAKATİ" MANTIĞI ####################
                # ##################################################################
                closest_locked_detection = None

                # 1. ÖNCELİK: Eğer zaten bir hedef kilitliyse, önce onu tekrar bulmaya çalış.
                if self.current_tracked_target_class is not None and not self.target_destroyed:
                    min_reacquisition_dist = float('inf')
                    last_known_center_x = self.current_tracked_target_bbox[0] + self.current_tracked_target_bbox[2] / 2
                    last_known_center_y = self.current_tracked_target_bbox[1] + self.current_tracked_target_bbox[3] / 2
                ...
                # 2. ÖNCELİK: Eğer kilitli hedefi bulamadıysak VEYA zaten bir hedefimiz yoksa, YENİ bir hedef ara.
                if closest_locked_detection is None and (
                        self.waiting_for_new_engagement_command or self.current_tracked_target_class is None):
                    candidate_target = None
                    minimum_distance = float('inf')

                    # GÖREVE ÖZEL YENİ HEDEF ARAMA MANTIĞI (SİZİN ORİJİNAL KODUNUZ, DEĞİŞTİRİLMEDİ)
                    if self.active_task == 'task1':
                        for det in detections:
                            det_center_x = det['bbox'][0] + det['bbox'][2] / 2
                            distance = abs(det_center_x - center_x_display)
                    ...
                    if candidate_target:
                        ...
                        self.target_destroyed = False
                        self.waiting_for_new_engagement_command = False
                        closest_locked_detection = candidate_target
                        self.reset_pid_state()
                        print(f"YENİ HEDEF KİLİTLENDİ: {self.current_tracked_target_class}")

                if prioritized_target_detection:
                            closest_locked_detection = prioritized_target_detection


                # --- HEDEF DURUMUNU YÖNETME VE TAHMİN ETME (SİZİN KODUNUZ, DEĞİŞTİRİLMEDİ) ---
                if closest_locked_detection:
                    self.missing_frames = 0
                    self.current_tracked_target_bbox = closest_locked_detection['bbox']
                    current_target_bbox_for_pid = self.current_tracked_target_bbox
                    ...
                    # Filtreyi ilk defa başlatma
                    if self.ema_cx is None:
                        self.ema_cx, self.ema_cy, self.ema_w, self.ema_h = pcx, pcy, pw, ph

                    # Tek karede büyük bir sıçrama var mı kontrol et
                    jump_dist = np.sqrt((pcx - self.ema_cx) ** 2 + (pcy - self.ema_cy) ** 2)

                    if jump_dist > self.LARGE_JUMP_PX:
                        # Sıçrama tespit edildi, hemen kabul etme, bekleme süresini başlat
                        self.jump_cooldown_until = current_frame_time + self.COOLDOWN_AFTER_LARGE_JUMP_MS / 1000.0

                        # Bu yeni konumu "aday" olarak izle
                        if self.candidate_jump_center is None:
                            self.candidate_jump_center = (pcx, pcy)
                            self.consecutive_stable_frames = 1
                        else:
                            # Adayın etrafında mı geziniyor, yoksa yine mi sıçradı?
                            if np.sqrt((pcx - self.candidate_jump_center[0]) ** 2 + (
                                    pcy - self.candidate_jump_center[1]) ** 2) < self.STABLE_RADIUS_PX:
                                self.consecutive_stable_frames += 1
                            else:  # Yeni bir yere sıçradı, adayı güncelle
                                self.candidate_jump_center = (pcx, pcy)
                                self.consecutive_stable_frames = 1

                        # Eğer bekleme süresi dolduysa VE yeterince istikrarlı kare gördüysek, YENİ KONUMU KABUL ET
                        if current_frame_time >= self.jump_cooldown_until and self.consecutive_stable_frames >= self.REQUIRE_CONSECUTIVE_STABLE_FRAMES:
                            self.ema_cx, self.ema_cy = self.candidate_jump_center
                            self.candidate_jump_center = None
                            self.consecutive_stable_frames = 0
                            print("SIÇRAMA KABUL EDİLDİ!")

                    else:  # Büyük sıçrama yok, normal EMA yumuşatması yap
                        ...
                        # Sıçrama bekleme durumundaysak ve süre dolduysa temizle
                        if self.candidate_jump_center and current_frame_time >= self.jump_cooldown_until:
                            self.candidate_jump_center = None
                            self.consecutive_stable_frames = 0

                    # PID'ye her zaman YUMUŞATILMIŞ (EMA) konumu gönder
                    current_target_bbox_for_pid = (self.ema_cx - self.ema_w / 2, self.ema_cy - self.ema_h / 2,
                                                   self.ema_w, self.ema_h)
                    self.current_tracked_target_bbox = current_target_bbox_for_pid  # Hafızadaki kutuyu da güncelle

                else:
                    self.missing_frames += 1
                    if self.missing_frames <= self.MAX_MISSING_FRAMES and self.current_tracked_target_class is not None:
                        if self.last_target_x is not None and self.last_frame_time is not None:
                            time_since_last = current_frame_time - self.last_frame_time
                            predicted_x = self.last_target_x + self.last_target_velocity_x * time_since_last
                            predicted_y = self.last_target_y + self.last_target_velocity_y * time_since_last
                            w_last, h_last = self.current_tracked_target_bbox[2], self.current_tracked_target_bbox[3]
                            current_target_bbox_for_pid = (int(predicted_x - w_last / 2), int(predicted_y - h_last / 2),
                                                           w_last, h_last)
                    else:
                        if self.current_tracked_target_class is not None:
                            print(f"HEDEF {self.current_tracked_target_class} KALICI OLARAK KAYBEDİLDİ.")
                            self.current_tracked_target_class = None
                            self.current_tracked_target_bbox = None
                            self.waiting_for_new_engagement_command = True
                            self.reset_pid_state()

            # EKRANA KUTULARI ÇİZME (SİZİN KODUNUZ, DEĞİŞTİRİLMEDİ)
            for det in detections:
            ...
            # KONTROL FONKSİYONUNU ÇAĞIRMA (SİZİN KODUNUZ, DEĞİŞTİRİLMEDİ)
            if current_target_bbox_for_pid and not self.target_destroyed:
                ...
                target_center_y = current_target_bbox_for_pid[1] + current_target_bbox_for_pid[3] / 2
            ...
            # GÖREVE ÖZEL DURUM GÜNCELLEMELERİ (SİZİN KODUNUZ, DEĞİŞTİRİLMEDİ)
            if self.active_task == 'task1':
                if current_target_bbox_for_pid and not self.target_destroyed:
                ...
                if self.missing_frames == 0 and current_target_bbox_for_pid and detected_class_status == "red_balloon" and self.is_aimed_at_target and not self.target_destroyed:
                    try:
                        self.fire_weapon()
                ...
                if self.is_in_no_fire_zone(self.current_yaw_angle):
                    self._update_status_label(
                        f"Durum: Tam Manuel Kontrol Modu - Ateşsiz Bölgede! Yaw: {self.current_yaw_angle:.1f}°")
                else:
                    self._update_status_label(f"Durum: Tam Manuel Kontrol Modu - Yaw: {self.current_yaw_angle:.1f}°")
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time):
        ...
        # 2. AKILLI HIZ VE İLERİ BESLEME (FEEDFORWARD) MANTIĞI
        feedforward_yaw = 0.0
        feedforward_pitch = 0.0

        # Adım 2a: Hız Hesaplama - SADECE hata küçükken (taret stabilken) hızı GÜNCELLE.
        if self.last_target_x is not None and abs(error_yaw_pixel) < self.velocity_update_error_threshold_px:
            raw_velocity_x = (target_x - self.last_target_x) / delta_time
            alpha = 0.4
            self.last_target_velocity_x = (alpha * raw_velocity_x) + (1 - alpha) * self.last_target_velocity_x
        # Hata büyükse, eski hız verisini yavaşça unut.
        else:
            self.last_target_velocity_x *= 0.95

            # Adım 2b: Feedforward Hesaplama - SADECE hız belirli bir eşiği geçerse kullan.
        ...
        # Pitch için de aynı mantık
        if self.last_target_y is not None and abs(error_pitch_pixel) < self.velocity_update_error_threshold_px:
            raw_velocity_y = (target_y - self.last_target_y) / delta_time
            alpha = 0.4
            self.last_target_velocity_y = (alpha * raw_velocity_y) + (1 - alpha) * self.last_target_velocity_y
        else:
            self.last_target_velocity_y *= 0.95
        ...
        # Adım 2c: Son konumları GÜNCELLE
        self.last_target_x = target_x
        self.last_target_y = target_y

        # 3. STANDART PID HESAPLAMALARI
        self.is_aimed_at_target = abs(error_yaw_pixel) <= self.aiming_tolerance

        self.integral_yaw += error_yaw_degree * delta_time
        self.integral_yaw = max(min(self.integral_yaw, 20.0), -20.0)
        derivative_yaw = (error_yaw_degree - self.last_error_yaw) / delta_time
        ...
        derivative_pitch = (error_pitch_degree - self.last_error_pitch) / delta_time
        self.last_error_pitch = error_pitch_degree

        output_yaw = (self.KP_YAW * error_yaw_degree + self.KI_YAW * self.integral_yaw +
                      self.KD_YAW * derivative_yaw + feedforward_yaw)
        output_pitch = (self.KP_PITCH * error_pitch_degree + self.KI_PITCH * self.integral_pitch +
                        self.KD_PITCH * derivative_pitch + feedforward_pitch)

        # 4. ÇIKIŞ KONTROLLERİ VE LİMİTLER
        ...
        # Bilgi etiketini güncelleme
        if self.missing_frames == 0:
            self.target_info_label.setText(f"Hedef: Kilitli. Hata: {error_yaw_pixel:.1f}px")
        else:
            self.target_info_label.setText(f"Hedef: Tahmin Ediliyor... ({self.missing_frames})")
