...
import sys
import cv2
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, \, QSlider, QSpinBox, QDoubleSpinBox, QDialogButtonBox, QLabel
...
CONF_THRESHOLD = 0.2  # Daha iyi tespit için güven eşiği düşürüldü
NMS_THRESHOLD = 0.4
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        self.is_turning_to_qr_angle = False
        self.target_yaw_from_qr = 0.0
        self.turn_start_time = 0.0
        self.task3_state = "IDLE"
        # ---- SAHI/Tiled (Task3 only) toggle & params ----
        self.use_sahi_task3 = False            # SAHI/Tiled sadece Task3 için aç/kapa
        self.tile_size_task3 = 512             # 384/512/640
        self.tile_overlap_task3 = 0.20         # 0.15–0.25
        self.tiled_conf_task3 = 0.15           # dilim içi skor eşiği
        self.tiled_iou_task3 = 0.45            # dilimler arası birleştirme IoU (NMS)
        self.tiling_every_n_frames_task3 = 1   # 1: her kare, 2: iki karede bir, ...
        self._frame_index_tiling_task3 = 0
  # "IDLE", "CONFIRMING_TARGET", "TURNING_TO_SECTOR", "SEARCHING_IN_SECTOR", "ENGAGING_TARGET", "RETURNING_TO_HOME"
        self.kill_confirmation_start_time = 0.0
        self.KILL_CONFIRMATION_DURATION_S = 1.0  # İmha onayı için 2 saniye bekle
        ...
        self.pitch_movement_limit = 10  # Varsayılan olarak limit yok
        ...
        self.CENTER_EMA_ALPHA = 0.45  # Merkez yumuşatma (düşük = daha yumuşak, yüksek = daha tepkisel)
        self.SIZE_EMA_ALPHA = 0.25  # Boyut yumuşatma
        self.LARGE_JUMP_PX = 75.0  # Tek karede merkez sıçrama eşiği (piksel)
        self.COOLDOWN_AFTER_LARGE_JUMP_MS = 150  # Sıçrama sonrası bekleme süresi (milisaniye)
        self.REQUIRE_CONSECUTIVE_STABLE_FRAMES = 2  # Sıçramanın geçerli sayılması için gereken istikrarlı kare sayısı
        self.STABLE_RADIUS_PX = 40.0  # "İstikrarlı" kabul edilme yarıçapı (piksel)

        # Filtrenin dahili durumlarını saklamak için değişkenler
        self.ema_cx, self.ema_cy, self.ema_w, self.ema_h = None, None, None, None
        self.jump_cooldown_until = 0.0
        self.candidate_jump_center = None
        ...
        # --- Kp MESAFE ÖLÇEKLEME ---
        self.enable_kp_distance_scaling = True  # İstersen kapat/aç
        self.kp_dist_near_px = 100  # ≤100 px → tam Kp
        self.kp_dist_far_px = 500  # ≥500 px → min ölçek
        self.kp_dist_min_scale = 0.10  # uzak hedefte Kp çarpanı (0.10)
        self.kp_dist_lpf_alpha = 0.5  # ölçek titreşimini yumuşat (0..1)
        self._kp_scale_prev = None  # dahili: LPF durumu
        ...
        self.movement_limits_group_box.setStyleSheet(
            "color: white; font-size: 16px; font-weight: bold; border: 2px solid white; border-radius: 8px; padding: 5px;")
        movement_limits_layout = QVBoxLayout()

        yaw_limit_layout = QHBoxLayout()
        yaw_limit_label = QLabel("Maksimum Yaw Sınırı (+/- °):")
        yaw_limit_label.setStyleSheet("color: white; font-size: 12px;")
        self.yaw_limit_input = QLineEdit(self)
        self.yaw_limit_input.setPlaceholderText("örn: 45.0")
        self.yaw_limit_input.setStyleSheet("color: black; background-color: white; font-size: 12px;")
        yaw_limit_layout.addWidget(yaw_limit_label)
        yaw_limit_layout.addWidget(self.yaw_limit_input)
        movement_limits_layout.addLayout(yaw_limit_layout)

        pitch_limit_layout = QHBoxLayout()
        pitch_limit_label = QLabel("Maksimum Pitch Sınırı (+/- °):")
        pitch_limit_label.setStyleSheet("color: white; font-size: 12px;")
        self.pitch_limit_input = QLineEdit(self)
        self.pitch_limit_input.setPlaceholderText("örn: 20.0")
        self.pitch_limit_input.setStyleSheet("color: black; background-color: white; font-size: 12px;")
        pitch_limit_layout.addWidget(pitch_limit_label)
        pitch_limit_layout.addWidget(self.pitch_limit_input)
        movement_limits_layout.addLayout(pitch_limit_layout)

        limit_buttons_layout = QHBoxLayout()
        self.apply_limits_button = QPushButton("Sınırları Uygula", self)
        self.apply_button_style(self.apply_limits_button, font_size=14, padding=6, bg_color="#007bff")
        self.clear_limits_button = QPushButton("Sınırları Temizle", self)
        self.apply_button_style(self.clear_limits_button, font_size=14, padding=6, bg_color="#6c757d")
        limit_buttons_layout.addWidget(self.apply_limits_button)
        limit_buttons_layout.addWidget(self.clear_limits_button)
...
# SAHI ayarları butonu (Task3)
self.btn_sahi_settings = QPushButton("SAHI ayarları")
self.btn_sahi_settings.setToolTip("SAHI/Tiled seçeneklerini açar (Task3)")
self.btn_sahi_settings.clicked.connect(self.open_sahi_settings_dialog)
task3_settings_layout.addWidget(self.btn_sahi_settings)
        ...
        # --- YENİ DÜZENLEME: Angajman Butonları ---
        task3_buttons_layout = QHBoxLayout()
        self.task3_start_button = QPushButton("Angajmanı Al", self)
        self.apply_button_style(self.task3_start_button, font_size=16, padding=8, bg_color="#28a745",
                                hover_color="#218838")  # Yeşil
        self.task3_stop_button = QPushButton("Angajmanı Durdur", self)
        self.apply_button_style(self.task3_stop_button, font_size=16, padding=8, bg_color="#dc3545",
                                hover_color="#c82333")  # Kırmızı
        task3_buttons_layout.addWidget(self.task3_start_button)
        task3_buttons_layout.addWidget(self.task3_stop_button)
        # --- BİTİŞ: Angajman Butonları ---
        ...
        task3_settings_layout.addLayout(task3_buttons_layout)  # Yatay buton düzenini ekle
        ...
        self.task3_stop_button.clicked.connect(self.stop_task3_engagement)

        # YENİ: Hareket Sınırları buton sinyalleri
        self.apply_limits_button.clicked.connect(self.apply_movement_limits)
        self.clear_limits_button.clicked.connect(self.clear_movement_limits)
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "CENTER_EMA_ALPHA": self.CENTER_EMA_ALPHA,
            "SIZE_EMA_ALPHA": self.SIZE_EMA_ALPHA,
            "LARGE_JUMP_PX": self.LARGE_JUMP_PX,
            "velocity_update_error_threshold_px": self.velocity_update_error_threshold_px,
        }
        try:
    ...
    def _kp_scale_by_distance(self, dist_px: float) -> float:
        """crosshair'e piksel uzaklığına göre Kp ölçeğini [0.1..1.0] arası döndürür."""
        near = float(self.kp_dist_near_px)
        far = float(self.kp_dist_far_px)
        smin = float(self.kp_dist_min_scale)

        if dist_px <= near:
            return 1.0
        if dist_px >= far:
            return smin
        # 100→500 px arası lineer ters orantı (1.0 → 0.1)
        t = (dist_px - near) / max(1.0, (far - near))  # 0..1
        return 1.0 - t * (1.0 - smin)
    ...
    # YENİ: Hareket Sınırlarını Uygulama Fonksiyonu
    def apply_movement_limits(self):
        try:
            yaw_limit_str = self.yaw_limit_input.text().strip()
            pitch_limit_str = self.pitch_limit_input.text().strip()

            # Boş girişleri "limit yok" olarak ayarla
            self.yaw_movement_limit = abs(float(yaw_limit_str)) if yaw_limit_str else 999.0
            self.pitch_movement_limit = abs(float(pitch_limit_str)) if pitch_limit_str else 999.0

            self._update_status_label(
                f"Durum: Hareket Sınırları: Yaw ±{self.yaw_movement_limit}°, Pitch ±{self.pitch_movement_limit}°")
            print(
                f"HATA AYIKLAMA: Hareket Sınırları ayarlandı: Yaw ±{self.yaw_movement_limit}°, Pitch ±{self.pitch_movement_limit}°")
        except ValueError:
            self._update_status_label("Hata: Lütfen sınırlar için geçerli sayılar girin.")
        except Exception as e:
            self._update_status_label(f"Hata: Sınırlar uygulanırken sorun: {e}")

    # YENİ: Hareket Sınırlarını Temizleme Fonksiyonu
    def clear_movement_limits(self):
        self.yaw_movement_limit = 999.0
        self.pitch_movement_limit = 999.0
        self.yaw_limit_input.clear()
        self.pitch_limit_input.clear()
        self._update_status_label("Durum: Hareket Sınırları Temizlendi.")
        print("HATA AYIKLAMA: Hareket Sınırları Temizlendi.")
    ...
    def open_settings_dialog(self):
        ...
        params = [
            ...
            ("Jitter Filtre Merkez Alfa", "CENTER_EMA_ALPHA", float),
            ("Jitter Filtre Boyut Alfa", "SIZE_EMA_ALPHA", float),
            ("Sıçrama Eşiği (px)", "LARGE_JUMP_PX", float),
            ("Akıllı Hız Güncelleme Eşiği (px)", "velocity_update_error_threshold_px", int),
            # ----------------------------------------
    ...
    def _process_rpi_response(self, response_data):
        ...
        if response_data.get("status") == "ok":
            if response_data.get("action") == "fire":
                self._update_status_label("Durum: Ateşleme Komutu RPi'ye Ulaştı!")
                print("Ateşleme Komutu RPi tarafından onaylandı.")

                # --- BAŞLANGIÇ: GÖREVE ÖZEL DAVRANIŞ ---
                # Sadece Aşama 1 veya 2 aktif ise hedefi yok edilmiş say.
                if self.active_task in ['task1', 'task2']:
                    ...
                    print("HATA AYIKLAMA: Aşama 1/2'de ateşlemeden sonra PID ve hedef bilgisi sıfırlandı.")

                # Aşama 3 aktif ise, hiçbir şey yapma. Onay sürecini bekle.
                elif self.active_task == 'task3':
                    # Bu durumda bayrağı biz ayarlamayacağız.
                    # İmha onayı durumu (CONFIRMING_KILL) bu kararı verecek.
                    print("HATA AYIKLAMA: Aşama 3'te ateşleme onayı alındı. İmha onayı bekleniyor.")
    ...
    def cancel_task(self):
        self.active_task = None
        self.is_engagement_active = False  # YENİ: Angajmanı durdur
        self.kcf_bbox = None
        self.kcf_active = False
        self.target_destroyed = False
        self.waiting_for_new_engagement_command = False
    ...
    def setup_task3(self):
        """Aşama 3'ü başlatır, tespitleri gösterir ama angajmanı başlatmaz."""
        self.cancel_task()
        ...
        if yolo_model_task3 is None:
            self._update_status_label("Hata: Aşama 3 modeli yüklenemedi.")
            self.cancel_task()
            return
        ...
        self._update_status_label("Durum: Aşama 3 - İzleme Modu. Angajman bekleniyor.")
        self.target_info_label.setText("Hedef Bilgisi: QR ve Hedefler İzleniyor.")
        print("HATA AYIKLAMA: Aşama 3 izleme moduna geçildi.")

    def start_task3_engagement(self):
        """'Angajmanı Al' butonuna basıldığında çalışır."""
        if self.active_task != 'task3':
            self._update_status_label("Hata: Aşama 3 aktif değil.")
            return
        if self.is_engagement_active:
            self._update_status_label("Uyarı: Zaten bir angajman aktif.")
            return
        ...
        try:
            ...
            self.is_engagement_active = True
            self.waiting_for_new_engagement_command = True
            self.target_destroyed = False
            self.reset_pid_state()
            self._update_status_label("Durum: Angajman Başlatıldı! QR kodu aranıyor...")
            print(f"HATA AYIKLAMA: Aşama 3 angajman başlatıldı. A:{a_degree}, B:{b_degree} dereceleri kaydedildi.")
        except ValueError:
            self._update_status_label("Hata: Lütfen geçerli sayısal angajman dereceleri girin.")

    def stop_task3_engagement(self):
        """'Angajmanı Durdur' butonuna basıldığında çalışır."""
        if self.active_task != 'task3' or not self.is_engagement_active:
            self._update_status_label("Uyarı: Aktif bir angajman yok.")
            return
        ...
        self.is_ready_to_engage_from_qr = False
        self.reset_pid_state()
        self._update_status_label("Durum: Angajman Durduruldu. İzleme moduna dönüldü.")
        print("HATA AYIKLAMA: Aşama 3 angajmanı manuel olarak durduruldu.")
    ...
    def _process_yolo_output(self, output, img_width, img_height, classes_list, conf_threshold=None):
        ...
        valid_predictions = predictions[scores > thresh]
        valid_scores = scores[scores > thresh]
        ...
        return [], [
    def open_sahi_settings_dialog(self):
        """SAHI/Tiled (Task3) ayarları için modal diyalog."""
        dlg = QDialog(self)
        dlg.setWindowTitle("SAHI ayarları — Task3")
        dlg.setModal(True)
        root = QVBoxLayout(dlg)

        help_lbl = QLabel("Küçük ve hareketli nesneler için dilimli (tiled) çıkarım.\nÖneri: Tile 384–640, Overlap %15–%25. FPS düşerse her N karede bir çalıştırın.")
        help_lbl.setWordWrap(True)
        help_lbl.setToolTip("SAHI: görüntüyü parçalara böler, her parçada YOLO çalıştırır ve sonuçları birleştirir.")
        root.addWidget(help_lbl)

        # Enable checkbox
        chk = QCheckBox("Task3 için SAHI/Tiled kullan")
        chk.setChecked(getattr(self, 'use_sahi_task3', False))
        chk.setToolTip("Task3 akışında dilimli çıkarımı etkinleştirir/kapatır.")
        root.addWidget(chk)

        # Tile size
        row1 = QHBoxLayout()
        row1.addWidget(QLabel("Tile size (px):"))
        sld_tile = QSlider(); sld_tile.setOrientation(1)  # Qt.Horizontal
        sld_tile.setRange(256, 1280); sld_tile.setSingleStep(32); sld_tile.setPageStep(64)
        sld_tile.setValue(int(getattr(self, 'tile_size_task3', 512)))
        spn_tile = QSpinBox(); spn_tile.setRange(256, 1280); spn_tile.setSingleStep(32)
        spn_tile.setValue(int(getattr(self, 'tile_size_task3', 512)))
        sld_tile.valueChanged.connect(spn_tile.setValue); spn_tile.valueChanged.connect(sld_tile.setValue)
        row1.addWidget(sld_tile); row1.addWidget(spn_tile); root.addLayout(row1)

        # Overlap
        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Overlap (%):"))
        sld_ov = QSlider(); sld_ov.setOrientation(1); sld_ov.setRange(5, 40); sld_ov.setSingleStep(1)
        cur_ov = int(round(float(getattr(self, 'tile_overlap_task3', 0.20))*100))
        sld_ov.setValue(max(5, min(40, cur_ov)))
        spn_ov = QDoubleSpinBox(); spn_ov.setDecimals(2); spn_ov.setRange(0.05, 0.40); spn_ov.setSingleStep(0.01)
        spn_ov.setValue(max(0.05, min(0.40, float(getattr(self, 'tile_overlap_task3', 0.20)))))
        sld_ov.valueChanged.connect(lambda v: spn_ov.setValue(v/100.0)); spn_ov.valueChanged.connect(lambda v: sld_ov.setValue(int(round(v*100))))
        row2.addWidget(sld_ov); row2.addWidget(spn_ov); root.addLayout(row2)

        # Every-N-frames
        row3 = QHBoxLayout()
        row3.addWidget(QLabel("Her N karede tiled:"))
        spn_every = QSpinBox(); spn_every.setRange(1, 10); spn_every.setValue(int(getattr(self, 'tiling_every_n_frames_task3', 1)))
        spn_every.setToolTip("1: her kare. FPS için 2–3 denenebilir.")
        row3.addWidget(spn_every); root.addLayout(row3)

        # Tiled conf
        row4 = QHBoxLayout()
        row4.addWidget(QLabel("Tiled conf:"))
        spn_conf = QDoubleSpinBox(); spn_conf.setDecimals(2); spn_conf.setRange(0.00,1.00); spn_conf.setSingleStep(0.01)
        spn_conf.setValue(float(getattr(self, 'tiled_conf_task3', 0.15)))
        row4.addWidget(spn_conf); root.addLayout(row4)

        # Merge IoU
        row5 = QHBoxLayout()
        row5.addWidget(QLabel("Birleştirme IoU:"))
        spn_iou = QDoubleSpinBox(); spn_iou.setDecimals(2); spn_iou.setRange(0.10,0.90); spn_iou.setSingleStep(0.01)
        spn_iou.setValue(float(getattr(self, 'tiled_iou_task3', 0.45)))
        row5.addWidget(spn_iou); root.addLayout(row5)
        ...
        def on_accept():
            setattr(self, 'use_sahi_task3', bool(chk.isChecked()))
            setattr(self, 'tile_size_task3', int(spn_tile.value()))
            setattr(self, 'tile_overlap_task3', float(spn_ov.value()))
            setattr(self, 'tiling_every_n_frames_task3', int(spn_every.value()))
            setattr(self, 'tiled_conf_task3', float(spn_conf.value()))
            setattr(self, 'tiled_iou_task3', float(spn_iou.value()))
            dlg.accept()
        btns.accepted.connect(on_accept); btns.rejected.connect(dlg.reject)
        dlg.exec_()
], []
    # ===== SAHI-Style Tiled Inference Helpers (Task3 only) =====
    def _nms_per_class(self, boxes, scores, class_ids, score_thresh, iou_thresh):
        """Sınıf-bazlı NMS (cv2.dnn.NMSBoxes kullanarak)."""
        if not boxes:
            return [], [], []
        ...
        for cid in sorted(set(class_ids)):
            ...
            if len(keep_idx) > 0:
                ...
                for i in keep_idx:
                    keep_boxes.append([int(round(v)) for v in boxes[i]])
                    keep_scores.append(float(scores[i]))
                    keep_cls.append(int(class_ids[i]))
        return keep_boxes, keep_scores, keep_cls

    def _detect_on_bgr(self, bgr_img, model, classes_list, conf_threshold=None):
        """Mevcut TRT/ONNX hattını tek bir BGR görüntü üzerinde çalıştırır ve kutu/score/sınıf listelerini döndürür."""
        global trt_context, trt_inputs, trt_outputs, trt_bindings, trt_stream, trt_engine, trt_output_binding_idx
        try:
            inp = self._preprocess_frame_for_yolo(bgr_img)
            if inp is None:
                return [], [], []
            # TensorRT vs ONNX Runtime
            if getattr(self, "model_is_tensorrt", False) and isinstance(model, str) and model == "tensorrt":
                if trt_context is None or trt_output_binding_idx is None:
                    return [], [], []
                import numpy as np
                ...
                outputs = model.run([output_name], {input_name: inp})
            h, w = bgr_img.shape[:2]
            boxes, confidences, class_ids = self._process_yolo_output(outputs[0], w, h, classes_list, conf_threshold=conf_threshold)
            return boxes, confidences, class_ids
        except Exception as e:
            print(f"HATA (_detect_on_bgr): {e}")
            return [], [], []

    def _process_yolo_detection_tiled_task3(self, frame, model, classes_list):
        """Task3 için SAHI benzeri tiled çıkarım: dilimle, model çalıştır, ofset ekle, sınıf-bazlı NMS ile birleştir."""
        H, W = frame.shape[:2]
        ts = int(getattr(self, "tile_size_task3", 512))
        overlap = float(getattr(self, "tile_overlap_task3", 0.20))
        step = max(1, int(ts * (1.0 - overlap)))
        conf_t = float(getattr(self, "tiled_conf_task3", 0.15))
        iou_t = float(getattr(self, "tiled_iou_task3", 0.45))
        ...
        for y0 in range(0, H, step):
            for x0 in range(0, W, step):
                ...
                bxs, scrs, clz = self._detect_on_bgr(tile, model, classes_list, conf_threshold=conf_t)
                for (x, y, w, h), s, c in zip(bxs, scrs, clz):
                    all_boxes.append([x + x0, y + y0, w, h])
                    all_scores.append(s)
                    all_cls.append(c)

        f_boxes, f_scores, f_cls = self._nms_per_class(all_boxes, all_scores, all_cls, conf_t, iou_t)

        detections = []
        for (x, y, w, h), s, c in zip(f_boxes, f_scores, f_cls):
            cname = classes_list[int(c)] if int(c) < len(classes_list) else "Bilinmeyen"
            detections.append({'bbox': (int(x), int(y), int(w), int(h)), 'score': float(s), 'class_name': cname})
        return detections
    ...
    def update_frame(self):
        ...
        try:
            ...
            if self.is_target_active and current_yolo_model is not None:
                # SAHI/Tiled sadece Task3 için (flag ile)

                if self.active_task == 'task3' and getattr(self, 'use_sahi_task3', False):

                    self._frame_index_tiling_task3 = getattr(self, '_frame_index_tiling_task3', 0) + 1

                    if int(getattr(self, 'tiling_every_n_frames_task3', 1)) <= 1 or (self._frame_index_tiling_task3 % int(getattr(self, 'tiling_every_n_frames_task3', 1)) == 0):

                        detections = self._process_yolo_detection_tiled_task3(frame, current_yolo_model, current_classes)

                    else:

                        detections = self.process_yolo_detection(frame, current_yolo_model, current_classes)

                else:

                    detections = self.process_yolo_detection(frame, current_yolo_model, current_classes)
# <-- DOĞRU: Orijinal 'frame' gönderiliyor
                ...
                if self.active_task == 'task3':
                    ...
                    if self.is_engagement_active:
                        ...
                        if self.task3_state == "IDLE":
                            ...
                            if data and data in self.qr_degrees:
                                ...
                                if closest_target_to_qr:
                                    ...
                                    self.confirmation_target_class = closest_target_to_qr['class_name']
                                    print(
                        ...
                        elif self.task3_state == "TURNING_TO_SECTOR":
                            self._update_status_label(f"Durum: Sektöre Dönülüyor ({self.target_yaw_from_qr:.1f}°)...")
                            if abs(self.current_yaw_angle - self.target_yaw_from_qr) < 1.5:
                            ...
                            if self.is_aimed_at_target:
                                ...
                                if current_time - self.last_fire_time > self.fire_cooldown_interval:
                                    if not self.is_in_no_fire_zone(self.current_yaw_angle):
                                        print(
                                            "HATA AYIKLAMA: Hedef nişan alındı, ATEŞ EDİLİYOR ve İMHA ONAYLAMA moduna geçiliyor.")
                                        self.send_command_to_rpi({"action": "fire"})
                                        self.last_fire_time = current_time
                                        self.task3_state = "CONFIRMING_KILL"
                                        self.kill_confirmation_start_time = time.time()

                            valid_targets = [d for d in detections if
                                             d['class_name'] == self.current_tracked_target_class]
                            closest_locked_detection = None
                            if valid_targets and self.current_tracked_target_bbox is not None:
                                last_center_x = self.current_tracked_target_bbox[0] + self.current_tracked_target_bbox[
                                    2] / 2
                                last_center_y = self.current_tracked_target_bbox[1] + self.current_tracked_target_bbox[
                                    3] / 2
                                closest_locked_detection = min(valid_targets, key=lambda d: np.hypot(
                                    (d['bbox'][0] + d['bbox'][2] / 2) - last_center_x,
                        ...
                        # --- DURUM: İMHA ONAYLAMA ---
                        elif self.task3_state == "CONFIRMING_KILL":
                            elapsed_time = time.time() - self.kill_confirmation_start_time
                            self._update_status_label(f"Durum: İmha onaylanıyor... ({elapsed_time:.1f}s)")
                            valid_targets = [d for d in detections if
                                             d['class_name'] == self.current_tracked_target_class]
                            is_target_still_visible = False
                            if valid_targets and self.current_tracked_target_bbox is not None:
                                last_center_x = self.current_tracked_target_bbox[0] + self.current_tracked_target_bbox[
                                    2] / 2
                                last_center_y = self.current_tracked_target_bbox[1] + self.current_tracked_target_bbox[
                                ...
                                for det in valid_targets:
                                    if np.hypot((det['bbox'][0] + det['bbox'][2] / 2) - last_center_x, (det['bbox'][1] +
                                                                                                        det['bbox'][
                                                                                                            3] / 2) - last_center_y) < self.MAX_REACQUISITION_DISTANCE_PIXELS:
                                        is_target_still_visible = True
                                        break
                    ...
                    if closest_locked_detection:
                        ...
                        # --- BİRLEŞTİRİLDİ: an2.py'den Jitter Filtresi Uygulaması ---
                        # 1. Ham (önerilen) sınırlayıcı kutuyu al
                        proposed_bbox = closest_locked_detection['bbox']

                        # 2. Bu ham kutuyu Jitter ve Sıçrama Filtresinden geçir
                        filtered_bbox = self._apply_jitter_filter(proposed_bbox, current_frame_time)

                        # 3. PID'ye gönderilecek hedef kutusu olarak FİLTRELENMİŞ kutuyu ata
                        current_target_bbox_for_pid = filtered_bbox

                        # Hız hesaplama ve diğer işlemler artık bu filtrelenmiş veriyi kullanacak
                        center_x_display = w // 2
                        target_center_x = filtered_bbox[0] + filtered_bbox[2] // 2
                        ...
                        if self.last_target_x is not None and self.last_frame_time is not None and error_pixel < self.velocity_update_error_threshold_px:
                            ...
                            if delta_time_for_velocity > 0:
                                ...
                                raw_velocity_y = (filtered_bbox[1] + filtered_bbox[
                        ...
                        self.last_target_y = filtered_bbox[1] + filtered_bbox[3] // 2
            ...
            if current_target_bbox_for_pid and not self.target_destroyed:
                # PID sadece görev 1,2 veya AKTİF ANGAJMANLI görev 3'te çalışır
                if (self.active_task in ['task1', 'task2']) or (
                        self.active_task == 'task3' and self.is_engagement_active):
            ...
            if self.active_task == 'task1':
                if current_target_bbox_for_pid and not self.target_destroyed:
                    ...
                    # ... (elif blokları aynı kalır) ...
            elif self.active_task == 'task3':
                # Sadece angajman aktifse durum mesajlarını güncelle
                if self.is_engagement_active:
    ...
    def _apply_jitter_filter(self, proposed_bbox, current_time):
        """an2.py'den alınan EMA ve sıçrama kontrol mantığı."""
        pcx, pcy = proposed_bbox[0] + proposed_bbox[2] / 2, proposed_bbox[1] + proposed_bbox[3] / 2
        pw, ph = proposed_bbox[2], proposed_bbox[3]

        if self.ema_cx is None:  # Filtreyi başlat
            self.ema_cx, self.ema_cy, self.ema_w, self.ema_h = pcx, pcy, pw, ph

        jump_dist = np.sqrt((pcx - self.ema_cx) ** 2 + (pcy - self.ema_cy) ** 2)

        if jump_dist > self.LARGE_JUMP_PX and current_time > self.jump_cooldown_until:
            self.jump_cooldown_until = current_time + self.COOLDOWN_AFTER_LARGE_JUMP_MS / 1000.0
        else:
            self.ema_cx = self.CENTER_EMA_ALPHA * pcx + (1 - self.CENTER_EMA_ALPHA) * self.ema_cx
            self.ema_cy = self.CENTER_EMA_ALPHA * pcy + (1 - self.CENTER_EMA_ALPHA) * self.ema_cy
            self.ema_w = self.SIZE_EMA_ALPHA * pw + (1 - self.SIZE_EMA_ALPHA) * self.ema_w
            self.ema_h = self.SIZE_EMA_ALPHA * ph + (1 - self.SIZE_EMA_ALPHA) * self.ema_h

        filtered_bbox = (self.ema_cx - self.ema_w / 2, self.ema_cy - self.ema_h / 2, self.ema_w, self.ema_h)
        self.current_tracked_target_bbox = filtered_bbox
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time):
        ...
        # 2) Radyal piksel uzaklığı (crosshair'e olan mesafe)
        dist_px = float((error_yaw_pixel ** 2 + error_pitch_pixel ** 2) ** 0.5)

        # 3) Mesafeye göre Kp ölçeği (0.10..1.0)
        kp_scale_raw = 1.0
        if getattr(self, "enable_kp_distance_scaling", False):
            kp_scale_raw = self._kp_scale_by_distance(dist_px)
            # İsteğe bağlı yumuşatma (LPF) → ani sıçramalar azalır
            if self._kp_scale_prev is None:
                kp_scale = kp_scale_raw
            else:
                a = float(self.kp_dist_lpf_alpha)
                kp_scale = a * kp_scale_raw + (1.0 - a) * self._kp_scale_prev
            self._kp_scale_prev = kp_scale
        else:
            kp_scale = 1.0

        # 4) Etkin Kp’ler: (yalnızca P terimini ölçekliyoruz)
        base_kp_yaw = self.KP_YAW
        base_kp_pitch = self.KP_PITCH
        eff_kp_yaw = base_kp_yaw * kp_scale
        eff_kp_pitch = base_kp_pitch * kp_scale
        ...
        output_yaw = (eff_kp_yaw * error_yaw_degree +
        ...
        output_pitch = (eff_kp_pitch * error_pitch_degree +
        ...
        # --- YENİ: Hareket Sınırları Kontrolü (Aşama 1 ve 2 için) ---
        if self.active_task in ['task1', 'task2']:
            # Tahmini yeni açıları hesapla
            predicted_yaw_after_move = self.current_yaw_angle + output_yaw
            predicted_pitch_after_move = self.current_pitch_angle + output_pitch

            # Yaw sınırı kontrolü
            if (predicted_yaw_after_move > self.yaw_movement_limit and output_yaw > 0) or \
                    (predicted_yaw_after_move < -self.yaw_movement_limit and output_yaw < 0):
                output_yaw = 0.0  # Yaw hareketini engelle
                self.integral_yaw = 0.0  # Sınıra ulaşıldığında integral birikimini önle
                print("UYARI: Yaw hareket sınırı aşıldı! Hareket engellendi.")

            # Pitch sınırı kontrolü
            if (predicted_pitch_after_move > self.pitch_movement_limit and output_pitch > 0) or \
                    (predicted_pitch_after_move < -self.pitch_movement_limit and output_pitch < 0):
                output_pitch = 0.0  # Pitch hareketini engelle
                self.integral_pitch = 0.0  # Sınıra ulaşıldığında integral birikimini önle
                print("UYARI: Pitch hareket sınırı aşıldı! Hareket engellendi.")
