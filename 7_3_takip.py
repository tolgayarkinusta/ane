# İLERİYE NİŞAN İÇİN LEAD_AIM_USE_FOR_PID = True YAP, LEAD_TIME_S = KAÇ SANİYE İLERİSİNE NİŞAN DEĞERİ

#Gimbal daha akışkan olsun istersen CENTER_EMA_ALPHA’yı biraz düşür (0.25–0.3).

#Hareketli hedef için çok “geriden geliyor” hissi olursa CENTER_EMA_ALPHA’yı 0.4–0.5 aralığına çıkar.

#Yan yana hedeflerde yanlış sık atlamalar varsa LARGE_JUMP_PX’i biraz küçült (60–70 px) ve REQUIRE_CONSECUTIVE_STABLE_FRAMES’i 3 yap.
import sys
import cv2
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, \
    QSpacerItem, QGroupBox, QLineEdit, QMessageBox, QRadioButton, QDialog
...
CONF_THRESHOLD = 0.1  # Daha iyi tespit için güven eşiği düşürüldü
NMS_THRESHOLD = 0.4
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        self.MIN_IOU_TO_UPDATE = 0.05  # Çok alakasız sıçramaları engellemek için alt IoU eşiği
        ...
        # --- Aşama 1–2 açı limiti ve home-dönüş davranışı ---
        self.TRACK_LIMIT_DEG = 30.0  # yaw/pitch için ±limit (ayarlanabilir)
        self.LOST_TO_HOME_TIMEOUT_S = 3.0  # 3 sn tespit yoksa 0.0 konuma dön
        self.RESUME_DELAY_AFTER_HOME_S = 2.0  # home'a varınca 2 sn bekle, sonra devam
        self.LIMIT_APPLIES_IN_TASKS = {1, 2}  # limitin geçerli olduğu aşamalar
        # Açı (komut) biriktiriciler: 0.0 etrafında çalışıyoruz
        self.tracking_accum_yaw_deg = 0.0
        self.tracking_accum_pitch_deg = 0.0
        # Kayıp/geri-dönüş durumu
        self.last_detection_time = time.perf_counter()
        self.return_state = "idle"  # "idle" | "going_home" | "resume_delay"
        self.resume_ready_at = 0.0
        ...
        # --- Jitter Filtresi (EMA) ve Sıçrama Kontrolü ---
        self.CENTER_EMA_ALPHA = 0.35  # 0..1 ; merkez yumuşatma (yüksek = daha az yumuşatma)
        self.SIZE_EMA_ALPHA = 0.25  # 0..1 ; genişlik/yükseklik yumuşatma
        self.LARGE_JUMP_PX = 80.0  # tek karede merkez sıçrama eşiği (px)
        self.COOLDOWN_AFTER_LARGE_JUMP_MS = 150  # sıçrama sonrasında 'kabul etmeden' önce bekleme (ms)
        self.REQUIRE_CONSECUTIVE_STABLE_FRAMES = 2  # sıçrama sonrası aynı yerde üst üste kaç kare görmek istiyoruz
        self.STABLE_RADIUS_PX = 40.0  # 'aynı yerde' kabulü için yarıçap (px)

        # --- Lead-Aim (ön nişan) ---
        self.LEAD_AIM_ENABLED = True  # overlay + opsiyonel PID
        self.LEAD_AIM_DRAW_OVERLAY = True  # ekranda lead noktasını çiz
        self.LEAD_AIM_USE_FOR_PID = False  # True yaparsan PID hedefi olarak lead kullanır
        self.LEAD_TIME_S = 0.20  # kaç saniye ilerisine nişan (0.15–0.30 tipik)
        # Hız eşiği: varsayılan FF eşiğini kullan ya da ayrı tanımla
        self.LEAD_SPEED_THRESHOLD_PX_S = float(getattr(self, "feedforward_velocity_threshold_px_s", 30.0))

        # EMA ve cooldown durumları
        self.ema_cx = None
        self.ema_cy = None
        self.ema_w = None
        self.ema_h = None
        ...
        self.feedforward_yaw_gain = 0.3  # Devre dışı bırakmak için 0.0 olarak ayarla
        self.feedforward_pitch_gain = 0.3  # Devre dışı bırakmak için 0.0 olarak ayarla
        ...
        # --- Türev filtresi (EMA) ve feed-forward sınırı ---
        self.DERIVATIVE_EMA_ALPHA = 0.20  # 0.0..1.0; daha düşük = daha fazla yumuşatma
    ...
    def _is_stage1_or_2(self):
        """
        Mevcut görev/aşama bilgisini projendeki değişken adına göre kontrol et.
        current_task (1/2/3) varsa onu kullan; yoksa konservatif varsayım.
        """
        return getattr(self, "active_task", None) in ("task1", "task2")

    def _apply_stage_limit_to_deltas(self, yaw_delta_deg, pitch_delta_deg):
        """
        Aşama 1–2'de ±TRACK_LIMIT_DEG sınırını aşırtmayacak şekilde
        delta komutlarını kırpar ve biriktiricileri günceller.
        """
        if not self._is_stage1_or_2():
            return yaw_delta_deg, pitch_delta_deg  # limit yok

        lim = float(self.TRACK_LIMIT_DEG)

        # YAW
        new_acc_yaw = self.tracking_accum_yaw_deg + yaw_delta_deg
        new_acc_yaw = max(-lim, min(lim, new_acc_yaw))
        yaw_delta_deg = new_acc_yaw - self.tracking_accum_yaw_deg
        self.tracking_accum_yaw_deg = new_acc_yaw

        # PITCH
        new_acc_pitch = self.tracking_accum_pitch_deg + pitch_delta_deg
        new_acc_pitch = max(-lim, min(lim, new_acc_pitch))
        pitch_delta_deg = new_acc_pitch - self.tracking_accum_pitch_deg
        self.tracking_accum_pitch_deg = new_acc_pitch

        return yaw_delta_deg, pitch_delta_deg

    def _update_last_detection_time(self, detections):
        """Her frame sonunda, en az bir geçerli tespit varsa 'son tespit zamanını' güncelle."""
        if detections and len(detections) > 0:
            self.last_detection_time = time.perf_counter()

    def _drive_home_step(self):
        """
        0.0 konuma doğru küçük adımlarla dön. Döndüyse True döndür.
        Bu, biriktiriciler üzerinden 'mevcut açı' tahminine göre çalışır.
        """
        # Mevcut hata (derece)
        err_yaw = -self.tracking_accum_yaw_deg
        err_pitch = -self.tracking_accum_pitch_deg

        # Yakınsayınca 'home'a vardık' kabulü (eşikleri istersen ayarla)
        if abs(err_yaw) < 1 and abs(err_pitch) < 1:
            return True

        # Her frame'de makul bir adım at: mevcut MAX_OUTPUT_DEGREE'ni kullan
        step_yaw = max(-self.MAX_OUTPUT_DEGREE, min(self.MAX_OUTPUT_DEGREE, err_yaw))
        step_pitch = max(-self.MAX_OUTPUT_DEGREE, min(self.MAX_OUTPUT_DEGREE, err_pitch))

        # Limit uygula (limitler içinde kalacağız, zaten 0.0 hedefi de limit içinde)
        step_yaw, step_pitch = self._apply_stage_limit_to_deltas(step_yaw, step_pitch)

        # Komutu gönder (projesindeki gönderim fonksiyonunu kullan)
        self.set_proportional_angles_delta(step_yaw, step_pitch)
        return False
    ...
    def _bbox_from_center_size(self, cx, cy, w, h):
        return (cx - w / 2.0, cy - h / 2.0, w, h)

    def _clip_point_to_frame(self, x, y, frame_w, frame_h):
    ...
    def _select_best_locked_match(self, detections, class_name, last_bbox, last_center_xy, max_center_dist_px):
        """Aynı sınıftaki adaylar arasından IoU+mesafe+güven bileşik skoruyla en iyi eşi döndürür."""
        best = None
        best_score = -1e9

        # Ağırlıklar: IoU baskın, sonra güven; merkez uzaklığı cezalandırılır
        W_IOU, W_CONF, W_DIST = 2.0, 0.5, 1.0

        for det in detections:
            if det.get('class_name') != class_name:
                continue
            db = det['bbox']
            dc = det.get('score', det.get('conf', 0.0))
            cx, cy = self._bbox_center(db)

            # Merkez mesafesi (tahmin edilen veya son merkezine göre)
            lx, ly = last_center_xy
            dist = ((cx - lx) ** 2 + (cy - ly) ** 2) ** 0.5
            if dist > max_center_dist_px:
                continue  # çok uzaksa ele

            iou = self._bbox_iou(last_bbox, db) if last_bbox else 0.0
            score = (W_IOU * iou) + (W_CONF * float(dc)) - (W_DIST * (dist / max(1.0, max_center_dist_px)))
            if score > best_score:
                best_score = score
                best = det
        return best
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "MIN_IOU_TO_UPDATE": self.MIN_IOU_TO_UPDATE,
            "KP_YAW": self.KP_YAW,
            "KI_YAW": self.KI_YAW,
            "KD_YAW": self.KD_YAW,
            ...
            "DERIVATIVE_EMA_ALPHA": self.DERIVATIVE_EMA_ALPHA,
    ...
    def open_settings_dialog(self):
        ...
        params = [
            ...
            ("MIN_IOU_TO_UPDATE", "MIN_IOU_TO_UPDATE", float),
            ...
            ("DERIVATIVE_EMA_ALPHA", "DERIVATIVE_EMA_ALPHA", float),
    ...
    def reset_pid_state(self):
        ...
        self.last_error_pitch = 0.0
        # --- yeni: türev filtresi tamponlarını sıfırla ---
        self.deriv_yaw_ema = 0.0
        self.deriv_pitch_ema = 0.0
        # --- Jitter/EMA ve cooldown sıfırlama ---
        self.ema_cx = self.ema_cy = self.ema_w = self.ema_h = None
        self.last_stable_center = None
        self.jump_cooldown_until = 0.0
        self.candidate_jump_center = None
        self.consecutive_stable_frames = 0
    ...
    def update_frame(self):
        ...
        try:
            ...
            if self.is_target_active and current_yolo_model is not None:
                ...
                self._update_last_detection_time(detections)
                ...
                if self.active_task == 'task3' and self.waiting_for_new_engagement_command and not self.is_ready_to_engage_from_qr:
                    ...
                    candidates_same_class = [d for d in detections
                                             if d.get('class_name') == self.current_tracked_target_class]

                    closest_locked_detection = self._select_best_locked_match(
                        candidates_same_class,
                        self.current_tracked_target_class,
                        self.current_tracked_target_bbox,
                        (last_tracked_center_x, last_tracked_center_y),
                        self.MAX_REACQUISITION_DISTANCE_PIXELS
                    )

                    if closest_locked_detection:
                        # --- JITTER FİLTRESİ + SIÇRAMA KONTROLÜ ---
                        use_time = current_frame_time  # zaten hesaplıyorsun; yoksa time.perf_counter() kullan
                        proposed_bbox = closest_locked_detection['bbox']
                        pcx, pcy = self._bbox_center(proposed_bbox)
                        pw, ph = proposed_bbox[2], proposed_bbox[3]

                        # EMA başlatma (ilk kez)
                        if self.ema_cx is None:
                            self.ema_cx, self.ema_cy = pcx, pcy
                            self.ema_w, self.ema_h = pw, ph
                            self.last_stable_center = (pcx, pcy)

                        # Mevcut EMA merkezinden tek karede ne kadar sıçradık?
                        from_ema_jump = ((pcx - self.ema_cx) ** 2 + (pcy - self.ema_cy) ** 2) ** 0.5

                        # --- Büyük sıçrama tespiti (yalnızca missing_frames==0 iken) ---
                        if self.missing_frames == 0 and from_ema_jump > self.LARGE_JUMP_PX:
                            # Cooldown penceresi başlat / uzat
                            self.jump_cooldown_until = max(self.jump_cooldown_until,
                                                           use_time + self.COOLDOWN_AFTER_LARGE_JUMP_MS / 1000.0)

                            # Bu noktayı 'aday sıçrama merkezi' olarak takip et
                            if self.candidate_jump_center is None:
                                self.candidate_jump_center = (pcx, pcy)
                                self.consecutive_stable_frames = 1
                            else:
                                # Aday etrafında istikrarlı mıyız?
                                dist_to_candidate = ((pcx - self.candidate_jump_center[0]) ** 2 +
                                                     (pcy - self.candidate_jump_center[1]) ** 2) ** 0.5
                                if dist_to_candidate <= self.STABLE_RADIUS_PX:
                                    self.consecutive_stable_frames += 1
                                else:
                                    # Aday yer değiştiyse, resetle
                                    self.candidate_jump_center = (pcx, pcy)
                                    self.consecutive_stable_frames = 1

                            # Cooldown bitmiş VE yeterince istikrarlı gördüysek: yeni konumu kabul et
                            if (use_time >= self.jump_cooldown_until and
                                    self.consecutive_stable_frames >= self.REQUIRE_CONSECUTIVE_STABLE_FRAMES):
                                self.ema_cx, self.ema_cy = self.candidate_jump_center
                                # boyutları da kabul et (ani scale zıplamasını biraz yumuşatmak istersek aşağıda EMA ile alırız)
                                ...
                                self.last_stable_center = (self.ema_cx, self.ema_cy)
                                # kabul ettik: adayları temizle
                                self.candidate_jump_center = None
                                self.consecutive_stable_frames = 0
                                self.jump_cooldown_until = 0.0

                            # Cooldown sürerken PID hedefi için **EMA tabanlı eski** konumu kullan (zıplama yapma)
                            current_target_bbox_for_pid = self._bbox_from_center_size(self.ema_cx, self.ema_cy,
                                                                                      self.ema_w, self.ema_h)

                        else:
                            # Büyük sıçrama YOK (veya missing_frames>0): normal EMA güncelle
                            ...
                            self.ema_h = self.SIZE_EMA_ALPHA * ph + (1 - self.SIZE_EMA_ALPHA) * self.ema_h
                            self.last_stable_center = (self.ema_cx, self.ema_cy)
                            # PID hedefi: EMA ile yumuşatılmış kutu
                            current_target_bbox_for_pid = self._bbox_from_center_size(self.ema_cx, self.ema_cy,
                                                                                      self.ema_w, self.ema_h)
                            # sıçrama adayı/cooldown temizliği
                            if use_time >= self.jump_cooldown_until:
                                self.candidate_jump_center = None
                                self.consecutive_stable_frames = 0
                                self.jump_cooldown_until = 0.0

                        # (İsteğe bağlı) Ekrana debug yaz
                        # print(f"[JITTER] jump={from_ema_jump:.1f}px, cooldown_left={max(0, self.jump_cooldown_until-use_time):.3f}s, "
                        #       f"stable_frames={self.consecutive_stable_frames}")
                        ...
                        if self.last_target_x is not None and self.last_frame_time is not None:
                            ...
                            if delta_time_for_velocity > 0:
                                self.last_target_velocity_x = (current_target_bbox_for_pid[0] +
                                                               current_target_bbox_for_pid[
                                                                   2] // 2 - self.last_target_x) / delta_time_for_velocity
                                self.last_target_velocity_y = (current_target_bbox_for_pid[1] +
                                                               current_target_bbox_for_pid[
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time,current_target_bbox_for_pid):
        ...
        if self._is_stage1_or_2():
            if self.return_state == "idle":
                # 3 sn tespit yoksa home'a geç
                if (now - self.last_detection_time) >= float(self.LOST_TO_HOME_TIMEOUT_S):
                    self.return_state = "going_home"
                    # PID integratör vs. taşımamak için küçük bir reset önerilir
                    # (istersen yorumu aç)
                    self.reset_pid_state()
            if self.return_state == "going_home":
                reached = self._drive_home_step()
                if reached:
                    self.return_state = "resume_delay"
                    self.resume_ready_at = now + float(self.RESUME_DELAY_AFTER_HOME_S)
                # Home'a dönüyorken tracking hesaplaması yapmıyoruz:
                return
            if self.return_state == "resume_delay":
                # Bekleme süresince komut gönderme (gimbal sabit kalsın)
                if now >= self.resume_ready_at:
                    self.return_state = "idle"
                    # Home'da sıfırlama (açı biriktiricileri ve PID)
                    self.tracking_accum_yaw_deg = 0.0
                    self.tracking_accum_pitch_deg = 0.0
                    self.reset_pid_state()
                else:
                    return
        # === Lead-Aim Debug Overlay ===
        # Parametre: hedefi kaç saniye sonrasına projekte edelim?
        self.LEAD_TIME_S = getattr(self, "LEAD_TIME_S", 0.20)  # 200 ms tipik; ayarlanabilir

        # === LEAD-AIM (ÖN NİŞAN) ===
        if getattr(self, "LEAD_AIM_ENABLED", False):
            # home/return sırasında kullanma
            if not hasattr(self, "return_state") or self.return_state == "idle":
                tx, ty, tw, th = current_target_bbox_for_pid
                cx = tx + tw * 0.5
                cy = ty + th * 0.5

                # Hedef hızları (px/s)
                vx = float(getattr(self, "last_target_velocity_x", 0.0))
                vy = float(getattr(self, "last_target_velocity_y", 0.0))
                speed_px_s = (vx * vx + vy * vy) ** 0.5

                use_lead = speed_px_s >= float(self.LEAD_SPEED_THRESHOLD_PX_S)

                # Öngörü merkezi
                pred_cx = cx + vx * float(self.LEAD_TIME_S)
                pred_cy = cy + vy * float(self.LEAD_TIME_S)
                pred_cx, pred_cy = self._clip_point_to_frame(pred_cx, pred_cy, frame_w, frame_h)

                # PID hedefini lead'e çek (opsiyonel)
                if use_lead and getattr(self, "LEAD_AIM_USE_FOR_PID", False):
                    # aynı boyutta sentetik bbox
                    current_target_bbox_for_pid = self._bbox_from_center_size(pred_cx, pred_cy, tw, th)

                # Overlay çiz
                if getattr(self, "LEAD_AIM_DRAW_OVERLAY", False):
                    # Yeşil: anlık tespit merkezi
                    cv2.circle(frame, (int(cx), int(cy)), 5, (0, 255, 0), -1)
                    # Sarı: lead noktası
                    lead_color = (0, 255, 255)
                    cv2.circle(frame, (int(pred_cx), int(pred_cy)), 5, lead_color, -1)
                    cv2.line(frame, (int(cx), int(cy)), (int(pred_cx), int(pred_cy)), lead_color, 2)
                    # Bilgi
                    cv2.putText(frame,
                                f"LEAD {('ON' if use_lead else 'OFF')} v={speed_px_s:.1f}px/s t={self.LEAD_TIME_S:.2f}s",
        ...
        # 1) Hızdan öngörü noktası (piksel)
        pred_cx = cx + self.last_target_velocity_x * self.LEAD_TIME_S
        pred_cy = cy + self.last_target_velocity_y * self.LEAD_TIME_S
        # 2) Çizimler
        #   Yeşil nokta: anlık tespit merkezi
        cv2.circle(frame, (int(cx), int(cy)), 5, (0, 255, 0), -1)
        #   Sarı nokta: öngörü (lead) noktası
        cv2.circle(frame, (int(pred_cx), int(pred_cy)), 5, (0, 255, 255), -1)
        cv2.line(frame, (int(cx), int(cy)), (int(pred_cx), int(pred_cy)), (0, 255, 255), 2)
        ...
        # --- FEED-FORWARD: hedefin piksel/s hızını derece/s'ye çevir, eşik üstünde uygula ---
        speed_px_s = (self.last_target_velocity_x ** 2 + self.last_target_velocity_y ** 2) ** 0.5
        if speed_px_s >= self.feedforward_velocity_threshold_px_s:
            ff_yaw_deg_s = self.last_target_velocity_x * self.DEGREES_PER_PIXEL_YAW
            ff_pitch_deg_s = self.last_target_velocity_y * self.DEGREES_PER_PIXEL_PITCH
            feedforward_yaw = self.feedforward_yaw_gain * ff_yaw_deg_s
            feedforward_pitch = self.feedforward_pitch_gain * ff_pitch_deg_s
            # İleri beslemeyi makul bir fraksiyonla sınırla
            ff_lim = self.MAX_OUTPUT_DEGREE * self.MAX_FEEDFORWARD_FRACTION
            feedforward_yaw = max(min(feedforward_yaw, ff_lim), -ff_lim)
            feedforward_pitch = max(min(feedforward_pitch, ff_lim), -ff_lim)
        else:
            feedforward_yaw = 0.0
        ...
        # --- TÜREV (EMA ile filtreli) ---
        derivative_yaw_raw = (error_yaw_degree - self.last_error_yaw) / delta_time if delta_time > 0 else 0.0
        alpha = self.DERIVATIVE_EMA_ALPHA
        self.deriv_yaw_ema = alpha * derivative_yaw_raw + (1.0 - alpha) * self.deriv_yaw_ema
        derivative_yaw = self.deriv_yaw_ema
        ...
        derivative_pitch_raw = (error_pitch_degree - self.last_error_pitch) / delta_time if delta_time > 0 else 0.0
        self.deriv_pitch_ema = alpha * derivative_pitch_raw + (1.0 - alpha) * self.deriv_pitch_ema
        derivative_pitch = self.deriv_pitch_ema
        ...
        if output_yaw != 0.0 or output_pitch != 0.0:
            ...
            # Aşama 1–2 için ±limit uygula
            send_yaw, send_pitch = self._apply_stage_limit_to_deltas(output_yaw, output_pitch)
            # Gönder
            self.set_proportional_angles_delta(send_yaw, send_pitch)
            if hasattr(self, "send_proportional_move_command"):
                self.send_proportional_move_command(send_yaw, send_pitch)
    ...
    def process_tracking_to_home_position(self):
        ...
        derivative_yaw_raw = (error_yaw_degree - self.last_error_yaw) / delta_time if delta_time > 0 else 0
        self.deriv_yaw_ema = self.DERIVATIVE_EMA_ALPHA * derivative_yaw_raw + (
                    1 - self.DERIVATIVE_EMA_ALPHA) * self.deriv_yaw_ema
        derivative_yaw = self.deriv_yaw_ema
        ...
        derivative_pitch_raw = (error_pitch_degree - self.last_error_pitch) / delta_time if delta_time > 0 else 0
        self.deriv_pitch_ema = self.DERIVATIVE_EMA_ALPHA * derivative_pitch_raw + (
                    1 - self.DERIVATIVE_EMA_ALPHA) * self.deriv_pitch_ema
        derivative_pitch = self.deriv_pitch_ema
