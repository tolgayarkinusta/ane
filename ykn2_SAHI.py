...
# SAHI KULLANIMI EKLENDİ. PARÇA PARÇA YOLO TESPİTİ YAPILIYOR. parametreleri bulmak için arat: (ctrl+f) --> self.use_sahi_task3
import sys
import cv2
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        # Aç/Kapa bayrağı (False: mevcut tek-kare çıkarım, True: SAHI tarzı tiled çıkarım)
        self.use_sahi_task3 = True
        # Tiled parametreleri (Task3)
        self.tile_size_task3 = 512           # 384/512/640 # FPS YETİYORSA DOĞRULUK İÇİN DÜŞÜR
        self.tile_overlap_task3 = 0.20       # 0.15–0.25
        self.tiled_conf_task3 = 0.15         # parça içi konf eşiği
        self.tiled_iou_task3 = 0.45          # parçalar arası birleştirme NMS IoU
        self.tiling_every_n_frames_task3 = 1 # 1: her karede, 2: iki karede bir, ... FPS ARTTIRMAK İÇİN ARTTIR
    ...
    def _detect_on_bgr(self, bgr_img, model, classes_list, conf_threshold=None):
        ...
        try:
            ...
            if inp is None:
                return [], [], []
            outputs = []
            # Mevcut kod mantığı: TensorRT vs ONNX Runtime
            ...
            h, w = bgr_img.shape[:2]
            # conf_threshold parametreli çağrı
        ...
        try:
            ...
            if self.active_task == 'task3' and getattr(self, 'use_sahi_task3', False):
                ...
                if int(getattr(self, 'tiling_every_n_frames_task3', 1)) <= 1 or                    (self._frame_index_tiling_task3 % int(getattr(self, 'tiling_every_n_frames_task3', 1)) == 0):
                    detections = self._process_yolo_detection_tiled_task3(display_frame, current_yolo_model, current_classes)
                else:
