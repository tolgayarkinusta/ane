...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, \
     QCheckBox, QDialog, QSlider, QSpinBox, QDoubleSpinBox, QDialogButtonBox, QSpacerItem, QGroupBox, QLineEdit, QMessageBox, QRadioButton, QDialog, QCheckBox
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        self.task3_state = "IDLE"  # "IDLE", "CONFIRMING_TARGET", "TURNING_TO_SECTOR", "SEARCHING_IN_SECTOR", "ENGAGING_TARGET", "RETURNING_TO_HOME"
        ...
        # SAHI/Tiled toggle (Task3 only)
        self.chk_sahi_task3 = QCheckBox("Task3 için SAHI/Tiled kullan")
        self.chk_sahi_task3.setVisible(False)
        self.chk_sahi_task3.setChecked(getattr(self, 'use_sahi_task3', False))
        self.chk_sahi_task3.toggled.connect(lambda v: setattr(self, 'use_sahi_task3', bool(v)))
        task3_settings_layout.addWidget(self.chk_sahi_task3)
    ...
    def _process_yolo_output(self, output, img_width, img_height, classes_list):
        ...
        return [], [], []
    # ===== SAHI Settings Dialog (Task3) =====
    def open_sahi_settings_dialog(self):
        """Opens a modal dialog to configure SAHI/Tiled settings for Task3."""
        dlg = QDialog(self)
        ...
        help_lbl.setToolTip("SAHI: görüntüyü parçalara böler, her parçada YOLO çalıştırır ve sonuçları birleştirir.\n"
                            "Küçük nesnelerde recall artışı sağlar.")
        root.addWidget(help_lbl)

        # Enable checkbox
        self.chk_sahi_task3_dlg = QCheckBox("Task3 için SAHI/Tiled kullan")
        self.chk_sahi_task3_dlg.setChecked(getattr(self, 'use_sahi_task3', False))
        self.chk_sahi_task3_dlg.setToolTip("Task3 akışında dilimli çıkarımı etkinleştirir/kapatır.")
        root.addWidget(self.chk_sahi_task3_dlg)

        # Tile size controls
        row1 = QHBoxLayout()
        row1.addWidget(QLabel("Tile size (px):"))
        self.sld_tile_size = QSlider()
        self.sld_tile_size.setOrientation(1)  # Qt.Horizontal
        self.sld_tile_size.setMinimum(256)
        self.sld_tile_size.setMaximum(1280)
        self.sld_tile_size.setSingleStep(32)
        self.sld_tile_size.setPageStep(64)
        cur_tile = int(getattr(self, 'tile_size_task3', 512))
        self.sld_tile_size.setValue(cur_tile)
        self.sld_tile_size.setToolTip("Dilim kenar uzunluğu (piksel). Küçük nesneler için 384–640 önerilir.")
        self.spn_tile_size = QSpinBox()
        self.spn_tile_size.setRange(256, 1280)
        self.spn_tile_size.setSingleStep(32)
        self.spn_tile_size.setValue(cur_tile)
        # sync
        self.sld_tile_size.valueChanged.connect(self.spn_tile_size.setValue)
        self.spn_tile_size.valueChanged.connect(self.sld_tile_size.setValue)
        row1.addWidget(self.sld_tile_size)
        row1.addWidget(self.spn_tile_size)
        root.addLayout(row1)

        # Overlap controls (percent scaled)
        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Overlap (%):"))
        self.sld_overlap = QSlider()
        self.sld_overlap.setOrientation(1)  # Qt.Horizontal
        self.sld_overlap.setMinimum(5)   # 5%
        self.sld_overlap.setMaximum(40)  # 40%
        self.sld_overlap.setSingleStep(1)
        cur_ov = float(getattr(self, 'tile_overlap_task3', 0.20))
        cur_ov_pct = max(5, min(40, int(round(cur_ov * 100))))
        self.sld_overlap.setValue(cur_ov_pct)
        self.sld_overlap.setToolTip("Dilim bindirmesi. 0.15–0.25 önerilir; sınırdaki nesneleri kaçırmamak için artırın.")
        self.spn_overlap = QDoubleSpinBox()
        self.spn_overlap.setDecimals(2)
        self.spn_overlap.setRange(0.05, 0.40)
        self.spn_overlap.setSingleStep(0.01)
        self.spn_overlap.setValue(cur_ov_pct / 100.0)
        # sync slider <-> spin (percent ↔ fraction)
        def _sync_overlap_from_slider(v):
            self.spn_overlap.setValue(v/100.0)
        def _sync_overlap_from_spin(v):
            self.sld_overlap.setValue(int(round(v*100)))
        self.sld_overlap.valueChanged.connect(_sync_overlap_from_slider)
        self.spn_overlap.valueChanged.connect(_sync_overlap_from_spin)
        row2.addWidget(self.sld_overlap)
        row2.addWidget(self.spn_overlap)
        root.addLayout(row2)
        ...
        self.spn_every.setValue(int(getattr(self, 'tiling_every_n_frames_task3', 1)))
        self.spn_every.setToolTip("1: her karede çalışır. FPS için 2–3 denenebilir.")
        row3.addWidget(self.spn_every)
        root.addLayout(row3)

        # Dialog buttons
        # Tiled conf (score threshold) controls
        row4 = QHBoxLayout()
        row4.addWidget(QLabel("Tiled conf:"))
        self.spn_tiled_conf = QDoubleSpinBox()
        self.spn_tiled_conf.setDecimals(2)
        self.spn_tiled_conf.setRange(0.00, 1.00)
        self.spn_tiled_conf.setSingleStep(0.01)
        self.spn_tiled_conf.setValue(float(getattr(self, 'tiled_conf_task3', 0.15)))
        self.spn_tiled_conf.setToolTip("Dilim içinde skor eşiği (0–1). Küçük nesnelerde recall için 0.10–0.25 tipik.")
        row4.addWidget(self.spn_tiled_conf)
        root.addLayout(row4)

        # Tiled merge IoU controls (postprocess match threshold)
        row5 = QHBoxLayout()
        row5.addWidget(QLabel("Birleştirme IoU:"))
        self.spn_tiled_iou = QDoubleSpinBox()
        self.spn_tiled_iou.setDecimals(2)
        self.spn_tiled_iou.setRange(0.10, 0.90)
        self.spn_tiled_iou.setSingleStep(0.01)
        self.spn_tiled_iou.setValue(float(getattr(self, 'tiled_iou_task3', 0.45)))
        self.spn_tiled_iou.setToolTip("Dilimler arası NMS eşik değeri (0–1). Duplikeler kalırsa düşür (örn. 0.40), silinirse yükselt.")
        row5.addWidget(self.spn_tiled_iou)
        root.addLayout(row5)
        ...
        def on_accept():
            # Write back to instance vars
            setattr(self, 'use_sahi_task3', bool(self.chk_sahi_task3_dlg.isChecked()))
            setattr(self, 'tile_size_task3', int(self.spn_tile_size.value()))
            setattr(self, 'tile_overlap_task3', float(self.spn_overlap.value()))
            setattr(self, 'tiling_every_n_frames_task3', int(self.spn_every.value()))

            setattr(self, 'tiled_conf_task3', float(self.spn_tiled_conf.value()))
            setattr(self, 'tiled_iou_task3', float(self.spn_tiled_iou.value()))
            # If there is an old checkbox in the panel, keep it in sync silently
            if hasattr(self, 'chk_sahi_task3') and self.chk_sahi_task3 is not None:
                self.chk_sahi_task3.setChecked(getattr(self, 'use_sahi_task3', False))
            dlg.accept()
