...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        # --- Uyarlamalı kontrol (hafif altyapı) ---
        self._ema_alpha = 0.5  # hız/ölçüm için EMA katsayısı
        self._last_cx = None
        self._last_cy = None
        self._speed_px_s = 0.0  # hedefin görüntüdeki hızı (px/sn)
        self._scale = 0.0  # hedefin ekrandaki bağıl boyutu (0..1)

        self.ANTI_WINDUP_KB = 2.0  # back-calculation kazancı (1.5–3 tipik)

        # (opsiyonel) gerçek balon çapını biliyorsan mesafe kestirimi için:
        self.camera_hfov_deg = 70.0  # kameranın yatay FOV'u
        self.target_real_diam_m = None  # örn 0.30  # bilmiyorsan None bırak
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time):
        """
        Hedef merkez koordinatlarına göre PID + FF kontrolü uygular ve RPi'ye orantılı hareket komutu gönderir.
        İyileştirmeler:
          - Anti-windup (back-calculation): Doyumda integral geri boşaltılır.
          - Türev (D) düşük geçiren filtre ile (EMA) hesaplanır (gürültü dayanıklı).
          - Gerçek feed-forward: Hedef piksel hızı + sistem gecikmesi ile "lead".
          - Rate limit (deg/s): Kare zamanına (dt) bağlı maksimum komut büyüklüğü.
        Not: Yeni parametreler __init__ gerektirmez; burada varsayılanları atanır.
        """

        # --- Çıkışın yapılmaması gereken durumlar ---
        if not self.rpi_thread.is_connected or self.active_task == 'full_manual' or self.target_destroyed:
            return

        # --- Varsayılanlar (gerekirse) ---
        # Anti-windup geri besleme kazancı
        if not hasattr(self, "ANTI_WINDUP_KB"): self.ANTI_WINDUP_KB = 2.0  # 1.5–3 aralığı tipik
        # Türev LPF kesim frekansı (Hz)
        if not hasattr(self, "D_LPF_HZ"): self.D_LPF_HZ = 12.0  # 8–15 Hz arası iyi başlar
        if not hasattr(self, "_deriv_yaw"): self._deriv_yaw = 0.0
        if not hasattr(self, "_deriv_pitch"): self._deriv_pitch = 0.0
        # Algılama->komut toplam gecikmesi (s)
        if not hasattr(self, "FF_LATENCY_S"): self.FF_LATENCY_S = 0.06  # ~60 ms tipik
        # Komut hız sınırlaması (deg/s)
        if not hasattr(self, "COMMAND_RATE_LIMIT_DPS"): self.COMMAND_RATE_LIMIT_DPS = 80.0
        # (Opsiyonel) nişan kararlılık süresi ölçümü için zaman damgası
        if not hasattr(self, "_aim_since"): self._aim_since = current_frame_time
        ...
        # --- Nişanda mı? (piksel toleransı ile) ---
        self.is_aimed_at_target = (
                abs(error_yaw_pixel) <= self.aiming_tolerance and
                abs(error_pitch_pixel) <= self.aiming_tolerance
        )
        # (Opsiyonel) kararlılık penceresi ölçümü – dışarıda kullanmak istersen
        if self.is_aimed_at_target:
            # hedef tolerans kutusunda kalmaya devam ederse zaman biriksin
            self._aim_since = getattr(self, "_aim_since", current_frame_time)
        else:
            self._aim_since = current_frame_time
        # aimed_ok = (current_frame_time - self._aim_since) >= 0.03  # istersen kullan

        # --- Zaman adımı ---
        dt = max(1e-3, current_frame_time - self.pid_update_time)
        self.pid_update_time = current_frame_time

        # --- Deadband: integral sıfırlamak yerine "koşullu entegrasyon" yapacağız.
        # Deadband içindeyse, hata e=0 alınır (integral sabit kalır), dışındaysa gerçek hata kullanılır.
        e_y = 0.0 if abs(error_yaw_degree) < self.pid_output_deadband_degree else error_yaw_degree
        e_p = 0.0 if abs(error_pitch_degree) < self.pid_output_deadband_degree else error_pitch_degree

        # --- Türev (D) – düşük geçiren filtre (EMA) ile gürültü bastırma ---
        # de/dt ham değeri:
        de_y = (e_y - self.last_error_yaw) / dt
        de_p = (e_p - self.last_error_pitch) / dt
        # EMA katsayısı: alpha = dt / (RC + dt), RC = 1/(2*pi*fc)
        RC = 1.0 / (2.0 * math.pi * self.D_LPF_HZ)
        alpha = dt / (RC + dt)
        # Filtreli türev
        self._deriv_yaw += alpha * (de_y - self._deriv_yaw)
        self._deriv_pitch += alpha * (de_p - self._deriv_pitch)
        # Son hata sakla
        self.last_error_yaw = e_y
        self.last_error_pitch = e_p

        # --- Feed-Forward (hedef hızına "lead") ---
        # Hedef piksel hızlarını update_frame'de zaten hesaplıyorsun (px/s).
        ff_yaw = 0.0
        ff_pitch = 0.0
        if abs(getattr(self, "last_target_velocity_x", 0.0)) > self.feedforward_velocity_threshold_px_s:
            ff_yaw = (self.feedforward_yaw_gain *
                      self.last_target_velocity_x *
                      self.DEGREES_PER_PIXEL_YAW * self.FF_LATENCY_S)
        if abs(getattr(self, "last_target_velocity_y", 0.0)) > self.feedforward_velocity_threshold_px_s:
            ff_pitch = (self.feedforward_pitch_gain *
                        self.last_target_velocity_y *
                        self.DEGREES_PER_PIXEL_PITCH * self.FF_LATENCY_S)

        # --- Integral (I) – koşullu entegrasyon (deadband dışındaki hata entegre edilir) ---
        # Not: burada integral "hata integrali" tutulur; çıkışa eklerken KI ile çarpılır.
        self.integral_yaw += e_y * dt
        self.integral_pitch += e_p * dt
        # Emniyet için kaba kelepçe (aşırı büyümeyi sınırlamak – gerçek anti-windup altta uygulanıyor)
        self.integral_yaw = max(min(self.integral_yaw, 20.0), -20.0)
        self.integral_pitch = max(min(self.integral_pitch, 20.0), -20.0)

        # --- P, I, D bileşenleri ---
        uP_y = self.KP_YAW * e_y
        uP_p = self.KP_PITCH * e_p
        uI_y = self.KI_YAW * self.integral_yaw
        uI_p = self.KI_PITCH * self.integral_pitch
        uD_y = self.KD_YAW * self._deriv_yaw
        uD_p = self.KD_PITCH * self._deriv_pitch

        # Ham çıktı (derece): bu "tek adımda" gönderilecek orantılı delta
        u_raw_y = uP_y + uI_y + uD_y + ff_yaw
        u_raw_p = uP_p + uI_p + uD_p + ff_pitch

        # --- Doyum (±MAX_OUTPUT_DEGREE) ---
        u_sat_y = max(min(u_raw_y, self.MAX_OUTPUT_DEGREE), -self.MAX_OUTPUT_DEGREE)
        u_sat_p = max(min(u_raw_p, self.MAX_OUTPUT_DEGREE), -self.MAX_OUTPUT_DEGREE)

        # --- Anti-windup (back-calculation):
        # Doyuma vurduysa integral terimini geri boşalt. Formül: I += (Kb*(u_sat - u_raw)/Ki)*dt
        if self.KI_YAW > 1e-9:
            self.integral_yaw += (self.ANTI_WINDUP_KB * (u_sat_y - u_raw_y) / self.KI_YAW) * dt
        if self.KI_PITCH > 1e-9:
            self.integral_pitch += (self.ANTI_WINDUP_KB * (u_sat_p - u_raw_p) / self.KI_PITCH) * dt

        # Anti-windup sonrası I terimini güncelleyip yeniden topla (nihai çıktı)
        uI_y = self.KI_YAW * self.integral_yaw
        uI_p = self.KI_PITCH * self.integral_pitch
        u_yaw = self.KP_YAW * e_y + uI_y + self.KD_YAW * self._deriv_yaw + ff_yaw
        u_pitch = self.KP_PITCH * e_p + uI_p + self.KD_PITCH * self._deriv_pitch + ff_pitch

        # --- Rate limit (deg/s): FPS düşse bile kare başına makul adım sınırı ---
        slew_max = self.COMMAND_RATE_LIMIT_DPS * dt
        u_yaw = max(min(u_yaw, slew_max), -slew_max)
        u_pitch = max(min(u_pitch, slew_max), -slew_max)

        # --- Minimum adım eşiği (çok küçük titremeleri öldür) ---
        if 0 < abs(u_yaw) < self.MIN_OUTPUT_DEGREE_THRESHOLD:  u_yaw = 0.0
        if 0 < abs(u_pitch) < self.MIN_OUTPUT_DEGREE_THRESHOLD:  u_pitch = 0.0

        # --- Aşama 3 için hareket kısıtlı bölge kontrolü (Yaw ekseni) ---
        if self.active_task == 'task3':
            predicted_yaw_after_move = self.current_yaw_angle + u_yaw
            if self.is_in_movement_restricted_zone(predicted_yaw_after_move):
                # Kısıtlı bölgeye girecekse yaw komutunu iptal et
                u_yaw = 0.0

        # --- Komut gönder ---
        if (u_yaw != 0.0) or (u_pitch != 0.0):
            self.send_proportional_move_command(u_yaw, u_pitch)
        else:
            # Nişan kutusundaysak bilgilendirici metin göster
            self.target_info_label.setText(
                f"Hedef: Nişan Alındı. Hata: Yaw {error_yaw_pixel}px, Pitch {error_pitch_pixel}px"
            )

        # --- Hedef izleme durum değişkenleri güncelle ---
        self.last_target_x = target_x
        self.last_target_y = target_y
        self.last_frame_time = current_frame_time

        # Bilgi etiketi (izleme durumu)
        self.target_info_label.setText(
